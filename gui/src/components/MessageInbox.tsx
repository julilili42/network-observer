...
interface MessageInboxProps {
  ...
  peers: PeerInfo[];
  selfPort: number;
  onLocalOutgoingMessage: (peer: PeerInfo, content: string) => void;
}
...
export function MessageInbox({
  ...
  }, [inbox, peers, selfPort]);

  useEffect(() => {
    if (!selectedPeer && allPeers.length > 0) {
      setSelectedPeer(allPeers[0]);
      return;
    }

    if (selectedPeer) {
      const refreshed = allPeers.find(
        (p) => p.ip === selectedPeer.ip && p.port === selectedPeer.port
      );

      if (refreshed) {
        setSelectedPeer(refreshed);
  ...
  const selectedMessages: PeerMessage[] = selectedPeer
    ? (inbox.find(
        ([peer]) =>
          peer.ip === selectedPeer.ip && peer.port === selectedPeer.port
      )?.[1] ?? [])
    : [];
  ...
  const handleSend = async () => {
    ...
    const ok = await api.sendPeerMessage(
      selectedPeer.ip,
      selectedPeer.port,
      content
    );
    ...
    if (ok) {
      onLocalOutgoingMessage(selectedPeer, content);
      setDraft("");
    }
  };

  const isActive = (peer: PeerInfo) => {
    const ageSecs =
      Math.floor(Date.now() / 1000) - peer.last_seen.secs_since_epoch;
    return ageSecs < 35;
  };
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {allPeers.length === 0 && (
            ...
            >
              Noch keine Nachrichten oder Peers.
              <br />
              Starte Discovery.
            </div>
          )}
          ...
          {allPeers.map((peer) => {
            ...
            const selected =
              selectedPeer?.ip === peer.ip && selectedPeer?.port === peer.port;
          ...
          <>
            ...
            >
              <div
                style={{
                  ...
                  height: 8,
                  borderRadius: "50%",
                  background: isActive(selectedPeer)
                    ? colors.accent
                    : colors.textMuted,
                }}
              />
            ...
            >
              {selectedMessages.length === 0 && (
                ...
                >
                  <Empty message="Noch keine Nachrichten in diesem Verlauf" />
                </div>
              )}
            ...
            >
              ...
              <Button
                onClick={handleSend}
                disabled={sending || !draft.trim() || !isActive(selectedPeer)}
                color={colors.purple}
  ...
  return (
    ...
    >
      ...
      >
        <span
          style={{
            ...
            fontSize: 10,
            fontWeight: 700,
            color: outgoing ? colors.accent : colors.purple,
          }}
        >
      ...
      <div
        style={{
          padding: "8px 12px",
          background: outgoing ? `${colors.accent}12` : `${colors.purple}12`,
          border: outgoing
